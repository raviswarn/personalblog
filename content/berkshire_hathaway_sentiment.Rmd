...
```{r plotting, fig.height=9, fig.width=10, error=F, message=F, warning=F, echo=F}
...
ggplot(letters_sentiment, aes(x = year, y = sentiment_pct)) + 
  ...
  labs(y='Net Sentiment Ratio',
       title='Text Sentiment of Berkshire Hathaway Letters to Shareholders, 1977 - 2016',
...
```{r sentiment_list, fig.height = 6, fig.width=10, error=F, message=F, warning=F, echo=F}
...
```{r wordcloud, fig.height = 10, fig.width = 10, error=F, message=F, warning=F, echo=F}
tidy_letters %>%
  filter(sentiment != 'neutral') %>%
  count(word, sentiment, sort = TRUE) %>%
...
If you're interested in reproducing this blog post or analysis, please check out the [R code I used to produce this document](www.google.com)